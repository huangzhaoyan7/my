...
def get_dataframe_from_excel():
    ...
    # 'supermarket_sales.xlsx'是Excel文件的路径及名称
    # sheet_name=''表示读取名为“销售数据”的工作表的数据
...
def hour_chart(df):
    ...
    # 使用px.bar函数生成条形图
    # - x=sales_by_hour.index：条形图的标签是小时数
    # - y="总价"：条形图的长度表示总价
    # - title：设置图表标题，使用HTML标签加粗
...
def main_page_demo(df):
    ...
    if df.empty:
        ...
        return  # 无数据时直接返回，避免后续计算报错

    # 选中数据框中的"总价"列，使用sum()计算"总价"列的和，使用int()求整
    total_sales = int(df["总价"].sum())
    # 计算平均评分（先删空值，再求平均）
    average_rating = df["评分"].dropna().mean()
    # 处理空值：如果平均评分为空，设为0；否则保留1位小数
    average_rating = round(average_rating, 1) if not pd.isna(average_rating) else 0
    # 生成星级字符串（空值时显示“暂无评分”）
    star_rating_string = ":star:" * int(round(average_rating, 0)) if average_rating != 0 else "暂无评分"
    # 选中数据框中的"总价"列，使用mean()计算"总价"列的平均值，使用round()四舍五入保留两位小数
